...
//I should convert all the prices to slippageRate????
//price() for main net
...
async function price() {
    ...
    let selectedColumns = array.map(({decimals,baseVolume,quoteVolume,lastTimestamp, ...keep}) => keep);
    let final = selectedColumns.map(({symbol: Symbol,name: Name, currentPrice: Buy, lastPrice: Sell,contractAddress: TokenAddress, ...rest}) => ({Symbol, Name, Buy, Sell,TokenAddress, ...rest}));
    return final
  };
...
price().then(function(data) {
       ...
       for (var i = 0; i < data.length; i++) {
        ...
        for (let i = 1; i < data.length+1; i++) {   // Click event listener for buy and sell
          //Used let for scooping.
          for (let j = 2; j < 3; j++) {
            table.rows[i].cells[j].addEventListener("click",function() {
              ...
              document.getElementById("tokenSource").value = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
          ...
          for (var j=3; j < 4; j++) {
            table.rows[i].cells[j].addEventListener("click",function() {
              ...
              document.getElementById("tokenAddressSell").value = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
...
function update() {
console.log("!")
main().then(function(data) {$('#example').DataTable( {
  data: data,
  columns: [
    { title:"Pair",data: 'symbol'},
    { title:"Name",data: 'name'},
    { title:"Price",data: 'currentPrice'},
    { title:"Volume",data: 'baseVolume'}
  ]});
  setTimeout(update,30000)}).catch(function (err) {
  console.log("err!");
...
/*var tokensInfo=main().then(function(a){
    return a;
  });*/
